...
def cal_curva(x, y, add=False):
    c = np.empty((1, 2, 20))
    if len(x) >= 3:
        k = len(x[2::2])
        if x[-1] == x[::2][-1] and add:
...
class Ui(QtWidgets.QMainWindow):
    ...
    def on_motion(self, event):
        ...
        x0, y0 = self.press
        if (x0[self.i] in x0[1::2]) and (y0[self.i] in y0[1::2]) and self.i + 1 != len(x0):
            x0[self.i + 2] = 2 * x0[self.i + 1] - event.xdata
            y0[self.i + 2] = 2 * y0[self.i + 1] - event.ydata
        if (x0[self.i] in x0[3::2]) and (y0[self.i] in y0[3::2]):
            x0[self.i - 2] = 2 * x0[self.i - 1] - event.xdata
            y0[self.i - 2] = 2 * y0[self.i - 1] - event.ydata
        if (x0[self.i] in x0[2::2]) and (y0[self.i] in y0[2::2]):
